...
contract ERC721 is Context, Ownable, ERC165, IERC721, IERC721Metadata {
     ...
     */
    constructor (string memory name_, string memory symbol_, string memory newBaseURI) {
        _name = name_;
        _symbol = symbol_;
        _baseURI = string(abi.encodePacked(newBaseURI, symbol_, "/"));
    }
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        if (bytes(_tokenUri).length > 0) {
            return string(abi.encodePacked(_baseURI, (tokenId.toString()), "/", _tokenUri));
        }
...
contract TikTokNft is ERC721Enumerable, Pausable {
    ...
    modifier onlyAdmin() {
        require(admins[msg.sender] == true, "not on the white list");
    ...
    /**
     * @dev Contract creation NFT
     *
     * @param _to address of owner
     * @param _tokenUri string of token id
     */
    function adminMint(address _to, string memory _tokenUri) external mintLock onlyAdmin returns(uint256) {
